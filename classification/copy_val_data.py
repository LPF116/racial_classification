...
val_destination_directory = 'fairface_clean/val'
test_destination_directory = 'fairface_clean/test'
...
with open('fairface_original/fairface_label_val.csv') as f:
    ...
    for race, list_of_files in new_dict.items():
        ...
        for file_info in new_dict[race]:
            if file_count <= 50:
                destination_race_directory = os.path.join(val_destination_directory, race)
                ...
                destination_race_directory = os.path.join(test_destination_directory, race)
                os.makedirs(destination_race_directory, exist_ok=True) 
                # print(f"{file_info=}")
                only_filename = file_info.split('/')[1]
                source_file = os.path.join(source_directory, file_info)
